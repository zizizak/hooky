...
Actions specifiy when an HTTP callback should be fired. The triggers that Hooky ships with are `CREATE`, `UPDATE`, and `DELETE`. Whenever these actions occur for a post type that you've specified, an HTTP callback will be sent to the endpoint you've specified. You can rely on the default structure for data to be sent in, or you can specify your own filters.
